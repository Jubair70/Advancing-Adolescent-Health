...
def get_adolescent_list(request):
    ...
    adolescent_query  = "with w as(with t as(select id as aid,hh_id, adolescent_name, id_adolescent, sex, age, have_birth_reg, relation, going_school, work_wage, disable, mobile, mobile_owner, maritial_status, marriage_reg, maritial_duration, child_any, child_num, pregnant_currently from plan_adolescents_profile), s as(select id,hh_head,village,union_name from plan_household_profile) select * from t,s where t.hh_id = s.id) select distinct on (id_adolescent) aid,union_name,hh_id, adolescent_name, id_adolescent, sex, age, have_birth_reg, relation, going_school, work_wage, disable, mobile, mobile_owner, maritial_status, marriage_reg, maritial_duration, child_any, child_num, pregnant_currently,hh_head,village from w where union_name::text = (select (select geocode from geo_data where id = geoid) from usermodule_catchment_area where user_id = (select id from auth_user where username = '"+username+"'))"
