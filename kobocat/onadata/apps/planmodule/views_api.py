...
def register_household(unregistered_hh):
    ...
    pngo_id = Organizations.objects.filter(organization=UserModuleProfile.objects.filter(user=created_user)[0].organisation_name)[0].id
...
def get_adolescent_list(request):
    ...
    adolescent_query  = "WITH w AS(WITH t AS (SELECT id AS aid, hh_id, adolescent_name, id_adolescent, sex, age, have_birth_reg, relation, going_school, work_wage, disable, mobile, mobile_owner, maritial_status, marriage_reg, maritial_duration, child_any, child_num, pregnant_currently FROM plan_adolescents_profile), s AS (SELECT id, district, upazila, union_name, mouza, village, para, hh_head, hh_no, pngo_id FROM plan_household_profile) SELECT * FROM t, s WHERE t.hh_id = s.id) SELECT DISTINCT ON (id_adolescent) aid, district, upazila, union_name, mouza, village, para, hh_head, hh_no, adolescent_name, id_adolescent, sex, age, have_birth_reg, relation, going_school, work_wage, disable, mobile, mobile_owner, maritial_status, marriage_reg, maritial_duration, child_any, child_num, pregnant_currently FROM w WHERE union_name :: text = (SELECT (SELECT geocode FROM geo_data WHERE id = geoid) FROM usermodule_catchment_area WHERE user_id = (SELECT id FROM auth_user WHERE username = '"+str(username)+"'))"
