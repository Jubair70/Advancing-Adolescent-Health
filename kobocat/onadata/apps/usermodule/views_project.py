...
def custom_project_window(request, username, id_string,**kwargs):
    ...
    if instance_id:
        query = "SELECT * from public.get_instance_extracted_temp(" + str(instance_id) + ")"
        form_data = __db_fetch_values_dict(query)
    
        omitFieldsLang = ['formhub/uuid', 'meta/instanceID', '_xform_id_string', 'username']
        first_field = ''
        for f in form_data:
