...
def delete_form_definition(request, form_definition_id):
    ...
    form_definition_calcuation(list_of_form_definition, form_definition_id)
    print(list_of_form_definition)
    return HttpResponseRedirect("/usermodule/geo_def_data/")


def form_definition_calcuation(list_of_form_definition, form_definition_id):
    ...
    database(delete_query)
    for each in id:
        form_definition_calcuation(list_of_form_definition, each)
####################### GEO CATCHMENT AREA #############################
...
def get_hh_list(request,username):
    ...
    sql = "select json->>'hh_id' id,(case json->>'is_respondent_contact_person' when '1' then json->>'respondent_name' else json->>'contact_person_name' end ) contact_person,(case json->>'is_respondent_contact_person' when '1' then json->>'hh_mobile_self' else json->>'contact_person_mobile' end ) mobile,'New' status from logger_instance where xform_id=400 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
    ...
    if tmp_db_value is not None:   
	   ...
	   if every[1] != None:
           	instance_data_json['contact_person'] = str(every[1].encode('utf-8'))
	   else:
		instance_data_json['contact_person'] = str(every[1])
	   if every[2] != None:
           	instance_data_json['mobile'] = str(every[2])
	   else:
		instance_data_json['mobile'] = str(every[2])
...
def get_union_list(request,username):
    ...
    sql = "select json->>'union_id' id,json->>'up_name' up_name,json->>'secretary_name' secretary,json->>'secretary_mobile' mobile,'New' status from logger_instance where xform_id=398 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
